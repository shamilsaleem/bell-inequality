...
% LOGO
\includegraphics[width=3.5cm]{images/collegelogo.png}\\[0.5cm]

\textbf{ST. JOSEPH'S COLLEGE (AUTONOMOUS), DEVAGIRI, CALICUT}\\[1cm]

\textbf{ “Beyond Classical Physics: Entanglement and Bell's Theorem”}\\[1cm]

Report of the project submitted to\\
St. Joseph's College (Autonomous), Devagiri\\
Affiliated to the University of Calicut\\[0.5cm]

By\\[0.5cm]

\textbf{ALAN A (DVAXSPH002)}\\
\textbf{DEVI KRISHNA A (DVAXSPH034)}\\
\textbf{LAKSHMIPRIYA RAMESH (DVAXSPH039)}\\
\textbf{NANDA R K (DVAXSPH040)}\\
\textbf{SHAMIL SALEEM.C (DVAXSPH044)}\\[0.5cm]

Program: B.Sc. Physics\\
Semester: VI\\[0.5cm]

\textbf{DEPARTMENT OF PHYSICS}\\[0.5cm]

March 2026\\[1cm]

\begin{flushleft}
Principal \hfill Head of the Department \hfill Supervisor
\end{flushleft}

\vfill
\justifying
\noindent College Seal
...
This is to certify that \textbf{ALAN A, DEVI KRISHNA A, LAKSHMIPRIYA RAMESH, NANDA R K and SHAMIL SALEEM.C} has worked on the topic “\textbf{Beyond Classical Physics: Entanglement and Bell's Theorem}” at St. Joseph’s College Devagiri (Autonomous), Calicut, under my supervision and guidance. This work has been carried out for the partial fulfilment of the Bachelor of Science Degree in Physics, University of Calicut.
...
\begin{flushleft}
\textbf{Dr. Meril Mathew} \hfill \textbf{Dr. Shinto P Mathew}\\
Head Of The Department \hfill Assistant Professor\\
Department Of Physics \hfill Department Of Physics\\
St. Joseph's College \hfill St. Joseph's College\\
(Autonomous), Devagiri \hfill (Autonomous), Devagiri\\
Calicut-673008 \hfill Calicut-673008\\
Kerala \hfill Kerala
\end{flushleft}
...
We hereby declare that this project entitled “\textbf{Beyond Classical Physics: Entanglement and Bell's Theorem}” submitted to St. Joseph’s College (Autonomous), Devagiri, affiliated to University of Calicut in partial fulfilment of the requirements for the award of degree of BACHELOR OF SCIENCE IN PHYSICS is a record of original project work done by us under the supervision and guidance of Dr. SHINTO P. MATHEW.

\vspace{2cm}

\begin{flushright}
ALAN A (DVAXSPH002)\\
DEVI KRISHNA A (DVAXSPH034)\\
LAKSHMIPRIYA RAMESH (DVAXSPH039)\\
NANDA R K (DVAXSPH040)\\
SHAMIL SALEEM.C (DVAXSPH044)\\
\end{flushright}

\vspace{1cm}

\noindent Kozhikode-673008\\
Date:
\vspace*{\fill}
\newpage
\vspace*{\fill}
...
At the very first we would like to express our deep-felt gratitude to the many hearts that worked behind this project. The constant encouragement, critical assessment and valuable guidance of Dr. SHINTO P. MATHEW gave us great insight during our work. We shall be ever grateful to him, and we are very grateful to Dr. MERIL MATHEW, Head of the Physics Department. We are also grateful to all the teachers and non-teaching staff of the Department of Physics for their good wishes and generosity.
...
\vspace{1em}
    This project investigates the fundamental differences between classical and quantum physics through the study of quantum entanglement, the EPR paradox, and Bell’s theorem. It examines the concepts of locality and realism and tests them using the CHSH inequality. Classical and quantum systems are analyzed through numerical simulations, showing that classical correlations satisfy the bound ${|S|\leq2}$, while quantum mechanics violates this limit, reaching values up to $2\sqrt{2}$

    A classical polarization experiment based on Malus’ Law is also conducted to provide experimental comparison. The results confirm that classical theories cannot explain quantum correlations, demonstrating the non-classical nature of entanglement and the limitations of local hidden-variable theories.
...
These limitations motivated the development of a more experimentally practical formulation. To overcome the experimental limitations of Bell’s original inequality, Clauser, Horne, Shimony, and Holt proposed a generalized form known as the \textbf{CHSH inequality}.
...
The optical arrangement used in the experiment is shown below:
...
In the experiment, the photodiode measures the photocurrent, which is directly proportional to the intensity of the transmitted light. However, the measured current depends not only on polarization effects but also on external factors such as fluctuations in laser power, detector sensitivity, and alignment errors.

To eliminate these effects, all measurements are normalized with respect to the maximum photocurrent obtained when the two polarizers are aligned.

Let $I_{\text{max}}$ denote the maximum measured photocurrent. For any pair of polarizer angles $a$ and $b$, the normalized transmission is defined as:
...
Here:
\begin{itemize}
    \item $I(a,b)$ is the measured photocurrent for polarizer settings $a$ and $b$,
    \item $T(a,b)$ represents the fraction of transmitted intensity.
\end{itemize}

This normalization removes dependence on absolute intensity and detector scaling factors.

The normalized transmission satisfies:
\begin{itemize}
    \item $T = 1$ corresponds to maximum transmission,
    \item $T = 0$ corresponds to zero transmission,
    \item $0 < T < 1$ corresponds to partial transmission.
\end{itemize}

Using Malus' Law, the transmission can be expressed as:
\begin{equation}
T(\theta) = \cos^2(\theta)
\end{equation}
...
To analyze the data within the CHSH framework, the measured quantity must be expressed as a correlation function ranging between $-1$ and $+1$. However, the normalized transmission $T(a,b)$ lies between $0$ and $1$.

To map this into the required range, we perform a linear transformation and define the correlation function as:
\begin{equation}
E(a,b) = 2T(a,b) - 1
\end{equation}

This transformation rescales the values as follows:
\begin{itemize}
    \item $T = 1 \rightarrow E = +1$
    \item $T = 0.5 \rightarrow E = 0$
    \item $T = 0 \rightarrow E = -1$
\end{itemize}

Physically, this means that higher transmission corresponds to stronger positive correlation, while lower transmission corresponds to negative correlation.

Substituting Malus' Law into the above expression:
\begin{equation}
...
This represents the theoretical prediction for correlations in a classical polarization experiment.
...
It should be noted that this experiment does not constitute a genuine Bell test, since the measurements are performed on a single classical light beam rather than on spatially separated entangled systems. Instead, it serves as a classical analogue of a Bell-type experiment, in which the mathematical structure of the CHSH inequality is reproduced using intensity measurements rather than independent measurement outcomes. Nevertheless, the experiment provides a useful classical baseline against which quantum mechanical violations of Bell-type inequalities can be compared.
\section{Conclusion}
This project explored the transition from classical physics to quantum mechanics and examined how our understanding of reality changes at the microscopic level. Classical physics assumes that physical properties exist with definite values whereas quantum mechanics introduced new principles such as superposition and entanglement that challenge these classical ideas.

The study of Quantum Entanglement showed that two particles can become strongly correlated so that the measurement of one particle instantly determines the state of the other, even when they are far apart. This phenomenon was highlighted in the EPR Paradox, proposed by Albert Einstein, Boris Podolsky, and Nathan Rosen. Their argument questioned quantum mechanics.

Later, John Bell developed Bell's Theorem, which provided a mathematical way to test whether hidden-variable theories could explain the correlations predicted by quantum mechanics. Bell derived inequalities that must be satisfied if local hidden variables exist. However, both theoretical predictions and experimental results show violations of these inequalities, indicating that nature does not follow the assumptions of classical locality and realism.

The introduction of the CHSH Inequality provided a more practical form of Bell’s inequality that could be tested experimentally. Simulations and experiments demonstrate that quantum mechanical predictions exceed the classical limits set by these inequalities, confirming the existence of non-classical correlations between entangled particles.

Overall, this study shows that the quantum world behaves in ways that cannot be fully explained using classical intuition. Thus, the investigation of entanglement, Bell’s theorem, and related experiments demonstrates that quantum mechanics provides a powerful and accurate description of the microscopic universe, while also opening new possibilities for future scientific and technological advancements.
...
\bibitem{beiser}
Arthur Beiser,
\textit{Concepts of Modern Physics},
6th Edition.

\bibitem{krane}
Kenneth S. Krane,
\textit{Modern Physics},
4th Edition.
